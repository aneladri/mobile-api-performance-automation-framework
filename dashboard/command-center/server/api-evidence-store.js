...
function createApiEvidenceStore({ root, maxEntries = 100 } = {}) {
  if (!root) throw new Error('API evidence store requires a root directory.');
  const directory = path.join(root, 'reports', 'command-center', 'api-evidence');
  ...
  function list() {
    const entries = fs.existsSync(directory) ? fs.readdirSync(directory).filter(name => name.endsWith('.json')) : [];
  ...
  function trim() {
    const entries = list();
    for (const evidence of entries.slice(maxEntries)) {
  ...
  return { directory, save, get, list };
}

module.exports = { createApiEvidenceStore };
